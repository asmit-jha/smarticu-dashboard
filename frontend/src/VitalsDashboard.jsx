...
const [avgBloodPressure, setAvgBloodPressure] = useState(null);
const [avgOxygenSaturation, setAvgOxygenSaturation] = useState(null);

useEffect(() => {
  const fetchLatestAverages = async () => {
    try {
      const response = await fetch("http://localhost:5000/api/vitals/average/latest");
      const data = await response.json();
      setAvgHeartRate(data.avgHeartRate);
      setAvgBloodPressure(data.avgBloodPressure);
      setAvgOxygenSaturation(data.avgOxygenSaturation);
    } catch (error) {
...
return (
  <div>
    <h3>10-Minute Averages</h3>
    <p>Average Heart Rate: {avgHeartRate || "Loading..."}</p>
    <p>Average Blood Pressure: {avgBloodPressure || "Loading..."}</p>
    <p>Average Oxygen Saturation: {avgOxygenSaturation || "Loading..."}</p>
  </div>
);